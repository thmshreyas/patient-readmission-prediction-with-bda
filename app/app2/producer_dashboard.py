...
try:
    ...
    # This topic feeds your HDFS Ingestion Consumer
    st.info("✅ Connected to Kafka topic 'patient_readmission'")
...
data = {
    'encounter_id': str(int(time.time())),
    'patient_nbr': str(int(time.time()) + 123),
    'race': race,
    'gender': gender,
    'age': age,
    'weight': "?",
...
if st.button("🚀 Send Data to Kafka"):
    try:
        producer.send("patient_readmission", data)
        producer.flush()
        st.success(f"✅ Data for encounter {data['encounter_id']} sent to Kafka topic 'patient_readmission'")
    except Exception as e:
